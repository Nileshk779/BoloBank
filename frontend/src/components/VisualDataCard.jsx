 ...
 */
export default function VisualDataCard({ data, big = false }) {
  if (!data) return null
  ...
  return (
    ...
    >
      ...
      >
        ...
        Shown on screen only — not spoken aloud
      </p>
      <p className={`font-semibold text-charcoal/70 ${big ? 'text-xl' : 'text-sm'}`}>{data.label}</p>
  ...
  if (data.type === 'transaction' && data.value && typeof data.value === 'object') {
    ...
    return (
      <div className="flex flex-col items-center gap-1">
        ...
        <p className={big ? 'text-lg text-charcoal/70' : 'text-sm text-charcoal/70'}>{description}</p>
