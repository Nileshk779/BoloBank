 ...
 */
export default function MicButton({ onRecordingComplete, disabled, big = false }) {
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div className={`relative flex items-center justify-center ${big ? 'h-64 w-64' : 'h-48 w-48'}`}>
        {recording && (
          <>
        ...
        <button
          ...
          className={`relative rounded-full flex items-center justify-center shadow-card
            transition-transform active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed
            ${big ? 'h-56 w-56' : 'h-40 w-40'}
            ${recording ? 'bg-secondary-600' : 'bg-primary-500 hover:bg-primary-600'}
            ${!recording && !disabled ? 'animate-pulseSoft' : ''}`}
        >
          {disabled && error === '' ? (
            <MicOff className={big ? 'h-24 w-24 text-white/70' : 'h-16 w-16 text-white/70'} strokeWidth={1.75} />
          ) : (
            <Mic className={big ? 'h-24 w-24 text-white' : 'h-16 w-16 text-white'} strokeWidth={1.75} />
          )}
        </button>
      </div>

      <p className={`font-semibold text-charcoal font-display ${big ? 'text-3xl' : 'text-lg'}`}>
        {recording ? (big ? 'Listening…' : 'Listening… release to send') : (big ? '🎙️ Hold to Speak' : 'Hold to speak')}
      </p>
      {error && <p className={`text-warn-600 font-medium text-center ${big ? 'text-xl max-w-md' : 'text-base max-w-xs'}`}>{error}</p>}
    </div>
  )
}
